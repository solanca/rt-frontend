...
function GooeyChart() {
  ...
  const strategyMap = useRef<any>();

  const getColor = (strategy: any) => {
    let hash = 0;
  ...
  const assignStrategyValues = (strat: any, alertType: any) => {
    if (!strategyMap.current[strat]) {
      strategyMap.current[strat] = Object.keys(strategyMap.current).length + 1;
    }
    return (
      strategyMap.current[strat] +
      (alertType.includes("customlong") ? 0.2 : 0.8)
    );
  };
      ...
      if (result && Array.isArray(result) && result.length > 0) {
        const newDataPoints = result.map((signal) => {
          ...
          return {
            ...
            strategyValue: assignStrategyValues(
              valueObj.strat,
              valueObj.alertType
            ),
        ...
        setData((prevData: any) => [...prevData, ...newDataPoints]);
      }
    }, 5000);
  ...
  return (
    ...
    >
      ...
      <XAxis dataKey="time" />
      <YAxis
        domain={[0, Object.keys(strategyMap.current).length + 1]}
        tickFormatter={(value: any) =>
          Object.keys(strategyMap.current).find(
            (key) => strategyMap.current[key] === Math.floor(value)
          )
        }
      />
      <Tooltip
        formatter={(_value: any, _name: any, props: any) => [
          props.payload.alertType,
          props.payload.strat,
        ]}
      />
      <Legend />
      ...
      {Array.from(strategies.current).map((strategy) => (
        <Line
          ...
          dot={{
            ...
            strokeWidth: 2,
            fill: (data: any) =>
              data.alertType.includes("customlong") ? "green" : "red",
          }}
        />
      ))}
    </LineChart>
