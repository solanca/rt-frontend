...
function OrderWidget() {
  ...
  // const [additionalSectionVisible, setAdditionalSectionVisible] =
  //   useState(false); // State to manage the visibility of additional section
  const [ExpireClose, setExpireClose] = useState(false);

  const handleMainTabChange = (_event:React.SyntheticEvent<Element, Event>, newValue:any) => {
    setMainTabValue(newValue);
  };

  const handleOrderTypeChange = (_event:React.SyntheticEvent<Element, Event>, newValue:any) => {
        ...
        <div className="order-widget-content">
          ...
          <Box>
            ...
            <Slider
              ...
              onChange={(_e, newValue) => setSpendPercentage(newValue as number)}
              step={5}
              marks
              min={0}
              max={100}
            ...
            <FormGroup>
              <FormControlLabel
                control={
                  <Switch
                    checked={ExpireClose}
                    onChange={(e) => setExpireClose(e.target.checked)}
                    size="small"
                  />
                }
          ...
          <Button
            variant="contained"
            color={mainTabValue === "buy" ? "success" : "error"}
            fullWidth
          >
